...
router.py               --> 规则匹配 + 熔断检查 + DNS 缓存
  ...
  +-- PROXY（境外IP / 规则代理）
          |
          v
      tunnel.py          --> TLS 连接池，默认 128 条并行隧道（可配置 1-128）
          |
          | 二进制帧协议（9 字节头）
          v
...
多条并行 TLS 1.2+ 连接承载多路复用流，通过二进制帧协议通信（大端序）。客户端使用连接池（默认 128 条，可配置 1-128），轮询分发流：
...
客户端与服务端之间维护 N 条并行 TLS 隧道（可配置为 1-128，默认 128）：
